...
var read_value = function (ctx, dview_func, byte_size, little_endian) {
  var value = ctx.getDataView()[dview_func](ctx.pos(), little_endian);
  ctx.skip(byte_size);
  return value;
}
...
module.exports = {
  ...
  bytes: function (length) {
    var dview = this.getDataView();
    var start = dview.byteOffset + this.pos();
    var value = dview.buffer.slice(start, start + length);
    this.skip(length);
    return value;
  }
}
